...
export async function finalizeArkFileUpload(
  ...
  return withArkResourceTransaction({
    ...
    const [upload] = await database.select().from(arkFileUploads).where(and(
    ...
    if (upload.status === 'finalized') {
      const [file] = await database.select().from(arkFiles).where(eq(arkFiles.id, upload.fileId)).limit(1)
      if (!file)
        throw new Error('Finalized upload is missing its Ark file.')
      return file
    }
    ...
    await database.update(arkFileUploads).set({
      etag: object.etag,
      finalizedAt: now,
      status: 'finalized',
      updatedAt: now,
