...
The per-invocation Hook Context exposes `database`, `schema`, and `accountability`, matching Directus. Service constructors and other extension dependencies belong to the surrounding registration scope rather than being added to every callback context. Metadata retains the useful Directus operation facts, including the Resource identifier, affected key or keys where applicable, and the relevant payload or query information for that event.
